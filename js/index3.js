...
function my_graph_draw(data) {
	...
	// ** Lets get the On-Time Airlines above 80% on-time for the selection
	
	//filteredData=SortedData.slice(0,200);
	filteredData=SortedData.filter(function(d) { 
	...
	// ** var svg = dimple.newSvg('#graph1',"100%", 400)
	// var svg = dimple.newSvg('#graph1',"50%", 400)
	...
	//**
	// ** myChart.setBounds(90, 50, 480, "79%"); 
    ...
    months.forEach(function (month) {
		...
		svg2
			...
			.style("opacity", 0.2)
			// ** .text(chartData[0].Month.substring(0, 3)); // ** Feedback 2
			.text((monthname[chartData[0].Month -1]));  // ** Feedback 2	  
	...
	d3.select("#submit").on("click", function() {
				...
				// ** Lets get the top 80% On-Time Airlines for the selection
			...
			d3.select("#graph2").select("svg").remove(); // **  but I like this one.
			// ** d3.select("#graph2").add();		// ** very dangerous, not going to use it
	...
	d3.select("#top5").on("click", function() {
			...
			*/
			...
			// ** Lets get the 80% On-Time Airlines for the selection
